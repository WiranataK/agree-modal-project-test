...
function iniData(){
    ...
    axios.get(process.env.REACT_APP_BACKEND_URL+'/api/retrievememberpartner?partner_code=KB', { headers: { Authorization: AuthStr } })
     .then(response => {
        // If request is good...
       arrMemberPartner = response.data;
       console.log(arrMemberPartner);
       fillTable();
     })
     .catch((error) => {
...
export const Table_Member_Partner = () => (
    <React.Fragment>
        <Row className="containers">
            ...
            <Row>
                <Table className="table" responsive striped hover variant="light" id="tableMemberPartner" onChange={iniData()}>
